<?php
	...
	$fp = fopen('ved_import_services.csv', 'r');
	$flag = 0;
	if ($fp) 
		{
		while (!feof($fp))
		...
		$out = split (";", $mytext);
		echo $mytext."<br />";
		$query = 'INSERT INTO ved_im_services ( year, CIS, Europe, EU, Asia, Africa, Aust_Ocean, America, Other) VALUES ('.$out[0].','.$out[1].','.$out[2].','.$out[3].','.$out[4].','.$out[5].','.$out[6].','.$out[7].','.$out[8].')';
		echo $query;
