<?php
	...
	if ($fp) 
		...
		$out = split (";", $mytext);
		echo $mytext."<br />";
		$query = 'INSERT INTO ved_services (year ,quartal,position, export_total ,export_to_pred_quartal,export_perxent_to_total,import_total,import_to_pred_quartal,import_perxent_to_total) VALUES (2016,1,"'.$out[0].'",'.$out[1].','.$out[2].','.$out[3].','.$out[4].','.$out[5].','.$out[6].')';
		echo $query;
